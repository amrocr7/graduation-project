...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<List<PrayerTime>> _calcPrayers() async {
    ...
    return PrayerCalculator(latitude: lat, longitude: lng, date: DateTime.now())
        .calculate();
  }
  ...
  void _tick() {
    ...
    for (final p in _prayers) {
      if (p.time.isAfter(now)) {
        setState(() {
          _nextPrayer = p;
          _nextPrayerCountdown = p.time.difference(now);
        });
        return;
      }
    }
  ...
  Future<void> _onPrayerTap(PrayerTime p, String status) async {
    ...
    if (p.time.isAfter(now)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'لم يحن وقت صلاة ${p.name.arabic} بعد!',
    ...
    if (index != -1) {
      if (index == _prayers.length - 1) {
        ...
        isCurrentActive = _prayers[index + 1].time.isAfter(now);
      }
    }
    ...
    if (isCurrentActive) {
      ...
      if (result == true) {
        ...
        final res = await HonorService.onPrayerDone(
            prayer: p.name, isOnTime: true, currentStreak: _streak);
        _showHonorChange(res.honor.history.first['delta'],
            res.honor.history.first['reason'], res.levelUp, res.levelDown);
        _load();
      }
    } else {
      showDialog(
          ...
          builder: (ctx) => AlertDialog(
                ...
                title: Text(p.name.arabic,
                    style:
                        const TextStyle(color: AppColors.danger, fontSize: 18)),
                content: const Text('صليتها أم فاتت؟',
                    style: TextStyle(color: AppColors.textPrimary)),
                actions: [
                  TextButton(
                      child: const Text('فاتت',
                      ...
                      onPressed: () async {
                        ...
                        Navigator.pop(ctx);
                        _showHonorChange(
                            res.honor.history.first['delta'],
                            res.honor.history.first['reason'],
                            res.levelUp,
                            res.levelDown);
                        _load();
                      }),
                  TextButton(
                      child: const Text('قضيتها ✓',
  ...
  void _showHonorChange(
      int delta, String reason, bool levelUp, bool levelDown) {
    setState(() => _honorChange =
        (delta: delta, reason: reason, levelUp: levelUp, levelDown: levelDown));
        ...
        Row(children: [
          GestureDetector(
            ...
            child: Container(
              ...
              child:
                  const Icon(Icons.grain, color: AppColors.primary, size: 20),
            ),
          ),
  ...
  Widget _streakCard() => GestureDetector(
        onTap: () => Navigator.push(
            context, MaterialPageRoute(builder: (_) => const StreakScreen())),
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
                color: _streak > 0 ? AppColors.primary : AppColors.border),
          ),
          child:
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _prayerTile(PrayerTime p) {
    ...
    final now = DateTime.now();
    final index = _prayers.indexOf(p);
    final isFuture = p.time.isAfter(now);
    ...
    final isPast = !isFuture && !isCurrentActive;
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            ...
            border: Border.all(
                color: bc,
                width: isDone || isMissed || isCurrentActive ? 1.5 : 1)),
        child:
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Row(children: [
  ...
  Widget _honorSection() => GestureDetector(
        onTap: () => Navigator.push(
            context, MaterialPageRoute(builder: (_) => const HonorScreen())),
        child: Container(
  ...
  Widget _bottomNav() =>
      Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
        ...
        _nb(
            ...
            'الشرف',
            () => Navigator.push(context,
                MaterialPageRoute(builder: (_) => const HonorScreen()))),
      ]);
